...
use App\models\Bankingoperator;
use App\models\Bankingtransaction;
use App\models\Broadbandoperator;
use App\models\Broadbandtransaction;
use App\models\Datacardoperator;
use App\models\Datacardtransaction;
use App\models\Dthconnectionoperator;
use App\models\Dthconntransaction;
use App\models\Dthtransaction;
use App\models\Electricityoperator;
use App\models\Electricitytransaction;
use App\models\Gasoperator;
use App\models\Gastransaction;
use App\models\Insuranceoperator;
use App\models\Insurancetransaction;
use App\models\Landlineoperator;
use App\models\Landlinetransaction;
use App\models\Mobiletransaction;
use App\models\Operator;
use App\models\Postpaidoperator;
use App\models\Postpaidtransaction;
use App\models\Prepaidoperator;
use App\models\Transaction;
use App\models\Waterbilloperator;
...
class Wallet
{
    ...
    static $mobile_redirecturl = 'http://zapwallet.in/mobilerechargeresponse';//Shared by CCAVENUES

    static $wallet_redirecturl = 'http://zapwallet.in/walletrechargeresponse';//Shared by CCAVENUES

    static $cancel_redirecturl = 'http://zapwallet.in/';

    private static $working_key = 'C999793EEB3E6535C376302DF5928B87';

    //protected $access_code = 'AVOQ72EH49BC73QOCB';
    ...
    public static function recharge_mobile ( $user, $rechargeamount, $currency, $tel, $operator, $fromwallet ) {
        ...
        if ( (bool) $fromwallet ) {
            if ( $rechargeamount > $user_wallet ) {
                ...
                $merchant_data = static::make_merchant_data($user, $order_id, $amount_for_gateway, self::$mobile_redirecturl, self::$cancel_redirecturl, $tel, $rechargeamount, $operator);
            ...
            $user->transactions()->save($transaction);
            $merchant_data = static::make_merchant_data($user, $order_id, $rechargeamount, self::$mobile_redirecturl, self::$cancel_redirecturl, $tel, $rechargeamount, $operator);
    ...
    private static function redirect_to_gateway ( $merchant_data ) {
        ...
        $encrypted_data = self::encrypt($merchant_data, static::$working_key); // Method for encrypting the data.
    ...
    public static function recharge_wallet ( $user, $currency, $amount, $redirecturl = 'http://zapwallet.in/walletrechargeresponse' ) {
        $order_id      = 'ZAP_' . time();
        $merchant_data = self::make_merchant_data($user, $order_id, $amount, $redirecturl, self::$cancel_redirecturl);
        $transaction   = new Transaction([
          'order_id' => $order_id,
          'currency' => $currency,
          'amount'   => $amount,
    ...
    public static function gateway_Response_Handler_mobile () {
        ...
        $encResponse       = $_POST[ "encResp" ];      //This is the response sent by the CCAvenue Server
        $rcvdString        = self::decrypt($encResponse, self::$working_key);    //Crypto Decryption used as per the specified working key.
        $decryptValues     = explode('&', $rcvdString);
        $response_order_id = explode('=', $decryptValues[ 0 ])[ 1 ];
        $tracking_id       = explode('=', $decryptValues[ 1 ])[ 1 ];
        $bank_ref_no       = explode('=', $decryptValues[ 2 ])[ 1 ];
